...
export const AnalisisMateria = ({ materia, periodo, grado, salon }: AnalisisMateriaProps) => {
  const { getPromediosEstudiantes, getPromediosSalones, getPromediosMaterias } = useEstadisticas();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <TablaEvolucion titulo={`Evolución de ${materia} por Período`} datos={evolucionMateria} />
      ...
      {grado && rendimientoPorSalon.length > 0 && (
        <ListaComparativa titulo={`Ranking de Salones - ${grado} - ${materia}`} items={rendimientoPorSalon.map(s => ({ nombre: s.salon, valor: s.promedio, extra: `${s.cantidadEstudiantes} estudiantes` }))} mostrarPosicion />
      )}
    </div>
  );
};
