        ...
        <div className="container mx-auto flex items-center justify-between gap-2">
          <Link to="/dashboard" className="flex items-center gap-2 md:gap-3 hover:opacity-80 transition-opacity cursor-pointer min-w-0">
            <img
              src={escudoImg}
              alt="Escudo"
              className="w-10 h-10 md:w-16 md:h-16 object-contain md:-my-2 flex-shrink-0"
            />
        ...
        <div className="bg-card rounded-lg shadow-soft overflow-hidden">
          ...
          <div className="flex border-b border-border">
            {periodos.map((periodo) => {
              ...
              const labelCorto = `${periodo.numero}° Per`;
              return (
                <button
                  key={periodo.numero}
                ...
                >
                  ...
                  <span className={`ml-1 md:ml-2 text-[10px] md:text-xs ${isActive ? 'text-primary-foreground/80' : 'text-muted-foreground'}`}>
                    ({porcentajeUsado}%)
                  </span>
                  {isActive && (
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-primary text-primary-foreground">
                    ...
                    <th className="md:sticky left-0 z-20 bg-primary border border-border/30 w-[80px] md:w-[100px] min-w-[80px] md:min-w-[100px] p-2 md:p-3 text-left font-semibold text-xs md:text-sm">
                      Código
                    </th>
                  ...
                  {estudiantes.map((estudiante, studentIndex) => {
                    ...
                    return (
                      <tr 
                        ...
                        className={rowBg}
                      >
                        {/* Fixed columns on desktop, NOT sticky on mobile */}
                        <td className={`md:sticky left-0 z-10 border border-border p-2 md:p-3 text-xs md:text-sm ${rowBg}`}>
                ...
                <tfoot>
                  <tr className="bg-muted/30">
                    {/* Celdas fijas vacías - NOT sticky on mobile */}
                    <td className="md:sticky left-0 z-10 bg-muted/30 border border-border p-1"></td>
