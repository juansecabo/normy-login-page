...
const TablaNotas = () => {
  ...
  const guardarFinalPeriodo = async (codigoEstudiantil: string, periodo: number, notaFinal: number | null) => {
    ...
    if (notaFinal === null) {
      ...
      // Limpiar comentario del estado local
      setComentarios(prev => {
        const updated = { ...prev };
        if (updated[codigoEstudiantil]?.[periodo]?.[finalActividadId]) {
          delete updated[codigoEstudiantil][periodo][finalActividadId];
        }
        return updated;
      });
    } else {
  ...
  const guardarFinalDefinitiva = async (codigoEstudiantil: string, notaFinal: number | null) => {
    ...
    if (notaFinal === null) {
      ...
      setComentarios(prev => {
        const updated = { ...prev };
        if (updated[codigoEstudiantil]?.[0]?.['0-Final Definitiva']) {
          delete updated[codigoEstudiantil][0]['0-Final Definitiva'];
        }
        return updated;
      });
    } else {
